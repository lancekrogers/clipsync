...
fn bench_encrypt_decrypt(c: &mut Criterion) {
    ...
    let encryptor = rt.block_on(async {
        Encryptor::new().await.unwrap()
    });
...
fn bench_compression(c: &mut Criterion) {
    ...
    group.bench_function("encrypt_less_compressible_1MB", |b| {
        b.iter(|| {
            let _encrypted = encryptor.encrypt(black_box(&less_compressible_1mb)).unwrap();
        });
    });
...
criterion_group!(benches, bench_encrypt_decrypt, bench_checksum, bench_compression);
