...
fn bench_history_operations(c: &mut Criterion) {
    ...
    for count in entry_counts {
        c.bench_with_input(
            ...
            &count,
            |b, &count| {
                let (_config, _clipboard, history, _sync_engine) =
                    rt.block_on(create_bench_setup());
    ...
    c.bench_function("concurrent_history_access", |b| {
        ...
        b.to_async(&rt).iter(|| async {
            ...
            for _ in 0..10 {
                ...
                let handle =
                    tokio::spawn(async move { history.get_recent_entries(10).await.unwrap() });
                handles.push(handle);
            }
    ...
    c.bench_function("memory_large_history", |b| {
        ...
        b.to_async(&rt).iter(|| async {
            ...
            for i in 0..1000 {
                let entry = clipsync::history::ClipboardEntry {
                    id: uuid::Uuid::new_v4(),
                    content: ClipboardData::Text(format!(
                        "Large history entry {} with more content to increase memory usage",
                        i
                    )),
                    timestamp: chrono::Utc::now(),
                    source: uuid::Uuid::new_v4(),
