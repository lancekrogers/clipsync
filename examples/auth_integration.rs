...
impl MockTransport {
    ...
    async fn handle_incoming_connection(&self, peer_pubkey: &PublicKey) -> Result<String, Box<dyn std::error::Error>> {
        println!("📞 Incoming connection from peer: {}", peer_pubkey.fingerprint());
        ...
        Ok(format!("Connection established with peer {}", peer_pubkey.fingerprint()))
    }
    
    /// Simulate validating a request with an auth token
    async fn validate_request(&self, token: &AuthToken) -> Result<PeerId, Box<dyn std::error::Error>> {
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Add ourselves to authorized keys for demo
    {
        ...
        auth_keys.save_to_file(&demo_dir.join("authorized_keys")).await?;
    ...
    match transport.handle_incoming_connection(&unauthorized_key).await {
    ...
    let token = transport.authenticator.authenticate_peer(&our_pubkey).await?;
