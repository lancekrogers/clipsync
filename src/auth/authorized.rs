...
impl AuthorizedKey {
    ...
    pub fn from_line(line: &str) -> Result<Self, AuthError> {
        ...
        if line.is_empty() || line.starts_with('#') {
            return Err(AuthError::InvalidKeyFormat("Empty or comment line".to_string()));
        }
        ...
        let key_data = parts.get(key_type_index + 1)
...
impl AuthorizedKeys {
    ...
    pub async fn save_to_file(&self, path: &Path) -> Result<(), AuthError> {
        ...
        file.write_all(b"# This file contains public keys authorized to connect to this ClipSync instance\n\n").await?;
    ...
    pub fn remove_key_by_fingerprint(&mut self, fingerprint: &str) -> bool {
        ...
        self.keys.retain(|k| k.public_key.fingerprint() != fingerprint);
    ...
    pub fn is_authorized(&self, public_key: &PublicKey) -> bool {
        ...
        self.keys.iter().any(|k| k.public_key.fingerprint() == fingerprint)
    }
    
    /// Get a key by fingerprint
    pub fn get_key_by_fingerprint(&self, fingerprint: &str) -> Option<&AuthorizedKey> {
        self.keys.iter().find(|k| k.public_key.fingerprint() == fingerprint)
    }
...
mod tests {
    ...
    async fn test_authorized_keys_save_load() {
        ...
        let key2_loaded = loaded.get_key_by_fingerprint(&key2.public_key().fingerprint()).unwrap();
