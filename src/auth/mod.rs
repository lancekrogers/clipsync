...
pub use authorized::{AuthorizedKeys, AuthorizedKey};
pub use keys::{KeyPair, PublicKey, KeyType};
pub use ssh::{SshAuthenticator, AuthToken, PeerId};
...
mod tests {
    ...
    fn test_default_auth_config() {
        ...
        assert!(config.private_key_path.to_string_lossy().contains("id_ed25519"));
        assert!(config.authorized_keys_path.to_string_lossy().contains("authorized_keys"));
