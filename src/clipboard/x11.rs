...
use super::{
    ...
    ClipboardSelection, ClipboardWatcher, MAX_CLIPBOARD_SIZE,
};
...
impl X11Clipboard {
    ...
    fn read_selection(&self, selection: ClipboardSelection) -> Result<Option<Vec<u8>>, ClipboardError> {
    ...
    fn write_selection(&self, selection: ClipboardSelection, data: &[u8]) -> Result<(), ClipboardError> {
        let atom = match selection {
            ClipboardSelection::Primary => self.clipboard.setter.atoms.primary,
            ClipboardSelection::Clipboard => self.clipboard.setter.atoms.clipboard,
        };
        ...
        if let Some(data) = self.read_selection(ClipboardSelection::Clipboard)? {
            ...
            if mime_type == "text/plain" {
                let text = String::from_utf8(data)
                    .map_err(|_| ClipboardError::Platform("Invalid UTF-8 in clipboard".to_string()))?;
