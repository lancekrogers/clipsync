...
impl FallbackDiscovery {
    ...
    async fn send_broadcast(
        ...
        let socket = socket_guard.as_ref().ok_or_else(|| anyhow!("No broadcast socket"))?;
        ...
        socket.send_to(&packet, ("255.255.255.255", BROADCAST_PORT)).await?;
    ...
    async fn connect_manual_peer(&self, manual_peer: &ManualPeer) -> Result<()> {
        ...
        // Add to peer manager
        self.peer_manager.add_peer(peer_info, DiscoveryMethod::Manual).await?;
...
impl Discovery for FallbackDiscovery {
    ...
    async fn discover_peers(&mut self) -> Result<Vec<PeerInfo>> {
        ...
        let mut peers = self.peer_manager
            .get_peers_by_method(DiscoveryMethod::Manual)
            .await?;
            
        let broadcast_peers = self.peer_manager
