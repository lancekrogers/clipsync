...
use crate::discovery::types::{PeerInfo, DiscoveryEvent, DiscoveryMethod};
use anyhow::Result;
...
impl PeerManager {
    ...
    pub async fn get_peers_by_method(
        &self,
        method: DiscoveryMethod,
    ) -> Result<Vec<PeerInfo>> {
        let peers = self.inner.peers.read().await;
...
mod tests {
    ...
    async fn test_add_and_get_peer() {
        ...
        manager.add_peer(peer.clone(), DiscoveryMethod::Mdns).await.unwrap();
