...
use crate::transport::{Connection, PeerInfo, Result, TransportError, TransportEvent};
...
impl ReconnectionManager {
    ...
    pub async fn start(&mut self) -> Result<()> {
        ...
        info!(
            "Starting reconnection manager for peer {}",
            self.peer_info.id
        );
    ...
    async fn attempt_connection(&mut self) -> Result<()> {
        ...
        info!(
            "Attempting connection to peer {} (attempt {})",
        ...
        match tokio::time::timeout(self.config.connection_timeout, self.connect_to_peer()).await {
            Ok(Ok(connection)) => {
    ...
    async fn attempt_reconnection(&mut self) -> Result<()> {
        if self.config.max_attempts > 0 && self.attempt_count >= self.config.max_attempts {
            error!(
                "Max reconnection attempts ({}) reached for peer {}",
                self.config.max_attempts, self.peer_info.id
            );
            return Err(TransportError::Reconnection {
                message: "Maximum reconnection attempts exceeded".to_string(),
            });
        }
    ...
    fn calculate_backoff_delay(&self) -> Duration {
        ...
        let backoff_delay = base_delay
            * self
                .config
                .backoff_multiplier
                .powi((self.attempt_count - 1) as i32);
...
mod tests {
    ...
    impl Authenticator for DummyAuth {
        async fn authenticate_peer(
            &self,
            _key: &crate::auth::PublicKey,
        ) -> std::result::Result<crate::auth::AuthToken, crate::auth::AuthError> {
        ...
        async fn verify_token(
            &self,
            _token: &crate::auth::AuthToken,
        ) -> std::result::Result<crate::auth::PeerId, crate::auth::AuthError> {
            Err(crate::auth::AuthError::AuthenticationFailed(
                "dummy".to_string(),
            ))
        }
        ...
        async fn get_public_key(
            &self,
        ) -> std::result::Result<crate::auth::PublicKey, crate::auth::AuthError> {
            Err(crate::auth::AuthError::AuthenticationFailed(
                "dummy".to_string(),
            ))
        }
