...
async fn test_network_interruption_recovery() {
    ...
    instance1
        .clipboard
        .set_text("During interruption")
        .await
        .unwrap();
    let sync_result = timeout(Duration::from_secs(2), instance1.sync_engine.sync_now()).await;
...
async fn test_multiple_device_sync() {
    ...
    for (i, instance) in instances.iter().enumerate() {
        ...
        assert_eq!(
            ...
            "Instance {} didn't receive message",
            i
        );
    }
}
...
async fn start_instance(config: Config) -> TestInstance {
    ...
    let sync_engine = Arc::new(
        SyncEngine::new(config, clipboard.clone(), transport.clone(), auth_manager)
            .await
            .unwrap(),
    );
