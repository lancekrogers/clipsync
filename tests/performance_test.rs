...
fn benchmark_clipboard_operations() {
    ...
    assert!(
        ...
        "Small text operations too slow"
    );
    ...
    assert!(
        ...
        "Medium text operations too slow"
    );
    ...
    assert!(
        ...
        "Large text operations too slow"
    );
}
...
fn benchmark_history_database() {
    ...
    runtime.block_on(async {
        ...
        let start = Instant::now();
        for i in 0..100 {
            db.search_entries(&format!("Entry {}", i * 100))
                .await
                .unwrap();
        }
        let duration = start.elapsed();
...
fn benchmark_memory_usage() {
    ...
    println!(
        "Memory growth during 100 operations: {} bytes",
        memory_growth
    );
